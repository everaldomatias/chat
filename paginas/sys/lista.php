<?php
	...
	$sql = "SELECT * FROM ( SELECT * FROM mensagens WHERE ( id_de = '$id_de' AND id_para = '$id_para' ) OR ( id_de = '$id_para' AND id_para = '$id_de' ) ORDER BY id DESC LIMIT 30 ) sub ORDER BY id ASC";
	$resultado = mysqli_query( $conecta, $sql );
	$conta = mysqli_num_rows( $resultado );
	
	if ( $conta <= 0 ) {
		echo "<code>A conversa não vai se iniciar sozinha. Manda um 'Oi'.</code>";
	} else {
		while ( $row = mysqli_fetch_array( $resultado ) ) :
			$row['mensagem'] = str_replace( $emotions, $images, $row['mensagem'] );
			if ( $row['id_de'] == $id_de ) { ?>
				<div class="pull-right chat-i">
					<?php echo $row['mensagem']; ?>
				</div>
				<div class="clearfix"></div>
			<?php } elseif( $row['id_de'] != $id_de ) { ?>
				<div class="pull-left chat-you">
					<?php echo $row['mensagem']; ?>
				</div>
				<div class="clearfix"></div>
			<?php } ?>
